...
class Runner {
  async run(input) {
    ...
    return { stdout: '8.0.36\tuuid-electron-mysql\tMySQL Community Server\n', exitCode: 0, stderr: '' };
  }
...
async function run() {
  ...
  const rootPath = await fs.mkdtemp(path.join(os.tmpdir(), 'deployerx-mysql-electron-test-'));
  const password = `electron-mysql-${Date.now()}-${Math.random()}`;
  ...
  try {
    ...
    const adapter = new MysqlLogicalAdapter({ processRunner: new Runner(), temporaryRoot: credentialRoot });
    const connections = new MysqlConnectionService({ controlDatabase, secretStore, deviceId: 'mysql-electron-device', adapter });
    const created = await connections.create('local', 'electron-test', { name: 'Electron MySQL', host: 'db.example.com', username: 'backup', password, tlsMode: 'verify-identity' });
    ...
    const source = await new DatabaseSourceService({ controlDatabase, adapterRegistry: new DatabaseAdapterRegistry([adapter]) }).save('local', 'electron-test', {
      name: 'Electron orders', connectionId: created.id,
    ...
    process.stdout.write(`${JSON.stringify({ ok, connectionStatus: tested.result.status, serverIdentity: tested.connection.trust.fingerprint, databases: discovered.items.map((item) => item.name), objects: objects.items.map((item) => item.name), sourceType: persistedSource.sourceType, plaintextPersisted: controlBytes.includes(Buffer.from(password)) || secretBytes.includes(Buffer.from(password)) })}\n`);
