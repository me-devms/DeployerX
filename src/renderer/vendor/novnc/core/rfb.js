...
export default class RFB extends EventTargetMixin {
    ...
    get screenLayout() {
        return this._screenLayout.map((screen) => ({ ...screen }));
    }

    get viewRegion() {
        return this._viewRegion ? { ...this._viewRegion } : null;
    }

    set viewRegion(region) {
        if (!region) {
            this._viewRegion = null;
        } else {
            const x = Math.max(0, Math.floor(Number(region.x) || 0));
            const y = Math.max(0, Math.floor(Number(region.y) || 0));
            const width = Math.min(Math.floor(Number(region.width) || 0), this._fbWidth - x);
            const height = Math.min(Math.floor(Number(region.height) || 0), this._fbHeight - y);
            this._viewRegion = width > 0 && height > 0 ? { x, y, width, height } : null;
        }
        this._updateClip();
        this._updateScale();
    }
    ...
    _updateClip() {
        ...
        if (this._viewRegion) {
            const region = this._viewRegion;
            this._display.clipViewport = true;
            this._display.viewportChangeSize(region.width, region.height);
            const viewport = this._display.viewport;
            this._display.viewportChangePos(region.x - viewport.x, region.y - viewport.y);
            this._setClippingViewport(true);
            if (!curClip) this._saveExpectedClientSize();
            return;
        }
    ...
    _sendEncodings() {
        ...
        if (this._fbDepth == 24) {
            // Prefer Tight/ZRLE in Electron. WebCodecs H.264 frames can remain
            // queued during disconnect and stall subsequent VNC sessions.
    ...
    _handleExtendedDesktopSize() {
        ...
        for (let i = 0; i < numberOfScreens; i += 1) {
            const screen = {
                id: this._sock.rQshift32(),
                x: this._sock.rQshift16(),
                y: this._sock.rQshift16(),
                width: this._sock.rQshift16(),
                height: this._sock.rQshift16(),
                flags: this._sock.rQshift32(),
            };
            screens.push(screen);
            if (i === 0) {
                this._screenID = screen.id;
                this._screenFlags = screen.flags;
            }
        }
        this._screenLayout = screens;
        ...
        this.dispatchEvent(new CustomEvent(
            "screenlayout", { detail: { screens: this.screenLayout } }));
    ...
    _resize(width, height) {
        ...
        this.dispatchEvent(new CustomEvent(
            "framebuffersize", { detail: { width: this._fbWidth, height: this._fbHeight } }));
